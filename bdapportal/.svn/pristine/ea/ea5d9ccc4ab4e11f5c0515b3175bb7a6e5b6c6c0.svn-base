...
public interface TblRepository extends JpaRepository<BdapTbl, String>,TblRepositoryCustom{
	...
	@Query(value="SELECT TBL_ID,TBL_KOR_NM, TBL_ENG_NM,TBL_IS_CHK_NULL,TBL_IS_CHK_TYPE,DATE_FORMAT(FROM_UNIXTIME(TBL_CREATE_DT/1000),'%Y-%m-%d %h:%i:%s') AS TBL_CREATE_DT,TBL_VALIDATE_DATE,TBL_IS_MANAGED,TBL_TYPE,TBL_DB_NM,TBL_DESC,TBL_RECENTLY_SYNC_DATE,"
			+ "(select sum(TBL_CNT) from MGMT_TBL_STAT where DB_NAME = TBL_DB_NM and TBL_NAME = TBL_ENG_NM) as TBL_OWNER, "
			+ "(select sum(tbl_size) from MGMT_TBL_STAT where DB_NAME = TBL_DB_NM and TBL_NAME = TBL_ENG_NM) as TBL_LOCATION FROM BDAP_TBL WHERE TBL_OWNER=?1 AND TBL_DB_NM=?2 ORDER BY CAST(TBL_ID AS UNSIGNED INTEGER) ASC ", nativeQuery = true)
